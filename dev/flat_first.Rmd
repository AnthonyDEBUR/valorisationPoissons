...
#' @param bar_width Largeur des barres
#' @param only_with_ipr TRUE = ne garder que les opérations ayant un IPR
#' @param n_last Nombre de campagnes distinctes à afficher
#'
#' @return ggplot2 ou NULL
#' @export
plot_effectifs_histogram <- function(
  yaml_path,
  station = NULL,
...
plot_surface_pechee_histogram <- function(
  ...
  if (use_station) {
    ...
    join_sql   <- ""   # ❗ JAMAMAIS de JOIN pour un CPP orphelin
    filtre_sql <- "o.code_point_prelevement_aspe = $1"
    param1     <- code_point_prelevement_aspe
  }
  ...
  # --------------------------------------------------------------------
  # 5) Sélection des n dernières dates
  # --------------------------------------------------------------------
  df <- df %>%
    distinct(date_op, .keep_all = TRUE) %>%
    arrange(desc(date_op)) %>%
...
plot_largeur_lame_eau_pechee_histogram <- function(
  ...
  # --------------------------------------------------------------------
  if (!is.null(code_station) && !is.null(code_point_prelevement_aspe)) {
    warning("Les deux paramètres code_station et code_point_prelevement_aspe sont fournis. Le code_station sera utilisé en priorité.")
  }
  use_station <- !is.null(code_station)
  ...
  if (use_station) {
    ...
    join_sql   <- ""   # ❗ aucun JOIN pour CPP orphelin
    filtre_sql <- "o.code_point_prelevement_aspe = $1"
    param1     <- code_point_prelevement_aspe
  }
  ...
  # --------------------------------------------------------------------
  # 5) Sélection des n dernières opérations (dates uniques)
  # --------------------------------------------------------------------
  df <- df %>%
    distinct(date_op, .keep_all = TRUE) %>%
    arrange(desc(date_op)) %>%
